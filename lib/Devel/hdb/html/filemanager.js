...
FileManager.prototype.markupPerlVars = function(line) {
    ...
    while (i < line.length) {
        ...
        if (/^(([$@])(\w+(::\w+)*)([[{]))/.test(remaining)) {
            ...
            var isSlice = RegExp.$2 === '@';
            var varName = RegExp.$3;
            var isArray = RegExp.$5 === '[';
            ...
            var arrayIndex = remaining.substring(varPrefix.length, endpos);
            output += '<span class="popup-perl-var" data-eval="'
                        + (isArray ? '@' : '%') + varName + '">' + varPrefix
                        + '</span><span class="popup-perl-var" data-eval="'
                        + remaining.substring(0, endpos + 1) + '">'
                        + arrayIndex + '</span>';
            i += endpos;
