function Debugger(sel) {
    ...
    function BreakpointManager() {
        ...
        function combinedBreakpointAction(filename, line) {
            ...
            if (line === undefined) {
                jQuery.extend(combined, breakpoints[filename], actions[filename]);

            } else {
                combined[line] = {};
                var bp = ((filename in breakpoints) && (line in breakpoints[filename]))
                        ? breakpoints[filename][line]
                        : {},
                    action = ((filename in actions) && (line in actions[filename]))
                        ? actions[filename][line]
                        : {};

                jQuery.extend(combined[line], bp, action);
            }
            return combined;
        };
        ...
        function makeUpdateMessageHandler(storage, code_key) {
            return function(data, is_delete) {
                var filename    = data.filename,
                    line        = data.lineno,
                    code        = data[code_key],
                    inactive    = data[ code_key + '_inactive'];
                ...
                if (is_delete) {
                    ...
                    storage[filename][line][code_key] = code;
                    storage[filename][line][code_key + 'Enabled'] = ! inactive;
                }
