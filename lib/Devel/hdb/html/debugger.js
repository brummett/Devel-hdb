        ...
        $elt.empty()
            ...
            .submit(function(e) {
                var form = e.target,
                    ...
                    isDebugListEdit = ! dbg.breakpointPopover;

                e.preventDefault();
                if (type === 'condition') {
                    dbg.breakpointManager.set({ filename: filename, lineno: lineno, condition: value});
                    $elt.empty().append( dbg.breakpointConditionTemplate({ condition: value }));

                } else if (type === 'action') {
                    dbg.breakpointManager.set({ filename: filename, lineno: lineno, action: value});
                    $elt.empty().append( dbg.breakpointActionTemplate({ action: value }));
                }
            });
    };
    ...
    function BreakpointManager() {
        ...
        this.breakpointUpdated = function(data) {
            ...
            if (action) {
                ...
                breakpoint[line].actionEnabled = ! conditionInactive;
            }
