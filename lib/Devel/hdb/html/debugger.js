function Debugger(sel) {
    ...
    this.setCurrentLineForCodeTable = function(codeTable, line) {
        ...
        var container = activeLine.closest('div#file-content');
    ...
    // the last time
    this.stackFrameChanged = function(i, frame, isSameFile) {
        var tabs = this.stackDiv.find('ul.nav'),
            panes = this.stackDiv.find('div.tab-content'),
            tabId = 'stack' + i,
            filename = frame.filename,
            line = frame.line,
            tab = tabs.find(':nth-child('+ (i+1) +')'),
            codePane = panes.find(':nth-child('+ (i+1) + ')'),
            dbg = this,
            html = '';
        ...
        if (frame) {
            html = this.navTabTemplate({label: frame.subname,
