            ...
            this.fileManager.loadFile(frame_obj.filename)
                .done(function($codeTableElt) {
                    ...
                    for ( var i = 0; i < frame_obj.args.length; i++) {
                        subArgs.push(('render' in frame_obj.args[i]) ? frame_obj.args[i].render('condensed') : frame_obj.args[i]);
                    }
    ...
    function pickFileToLoad(e) {
        ...
        function renderNewFile($codeElt, filename, line) {
            var tabId = generateNewTabId(),
                ...
                $container = $( that.templates.navPane({ uuid: tabId, filename: filename, banner: filename })),
