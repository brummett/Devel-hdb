function Debugger(sel) {
    ...
    function _QB_findBreakableLineInSub(requested_line, deferred, subinfo_data) {
        var start_breakable_search;

        if (requested_line) {  // They wanted to stop on a particular line
            var sub_length = subinfo_data.end - subinfo_data.line;
            if (requested_line > sub_length) {
                deferred.reject(subinfo_data.subroutine + ' is only ' + sub_length + ' lines');
            }
            // -1 because we should start searching on the first line of the sub, which
            // could be on the same line as the 'sub' declaration
            start_breakable_search = subinfo_data.line + requested_line - 1;

        } else {
            start_breakable_search = subinfo_data.line;
        }
