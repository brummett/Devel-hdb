    ...
    function renderPart(value) {
        ...
        if ((value === undefined) || (value === null)) {
            ...
            if ($.isArray(value.__value)) {
                html += '<span class="badge badge-info">' + value.__value.length
                        + '</span></span><dl>';
                for (k = 0; k < value.__value.length; k++) {
                    html += '<dt>'+ k +'</dy><dd>' + renderPart(value.__value[k])
                                + '</dd>';
                }
                html += '</dl>';
            }
                ...
                for (k in value.__value) {
                    count++;
                    subhtml += '<dt>' + k + '</dt><dd>' + renderPart(value.__value[k])
                            + '</dd>';
                }
                html += '<span class="badge badge-info">' + count + '</span></span><dl>'
                        + subhtml + '</dl>';
            }
            else {
                debugger;
