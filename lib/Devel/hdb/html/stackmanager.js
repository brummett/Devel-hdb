    ...
    this.update = function() {
        ...
        var finalize = function() {
            ...
            for (var serial in prev_frames_by_serial) {
                d.notify(prev_frames_by_serial[serial], prev_frames_by_serial[serial].frameno, undefined);
            }
            frames = new_frames;
            d.resolve();
        };
        ...
        function update_one_frame(frame, i) {
            frame.frameno = i;
            new_frames[i] = frame;
            if (frame.serial in prev_frames_by_serial) {
                // this frame existed before
                var old_frameno = prev_frames_by_serial[frame.serial].frameno;
                d.notify(frame, old_frameno, i);
                delete prev_frames_by_serial[frame.serial];

            } else {
                // This is a new frame that did not exist before
                d.notify(frame, undefined, i);
            }
        };
